...
public class MeetBip1PollinationHappeningState extends BeetrapState {
    ...
    public MeetBip1PollinationHappeningState(BeetrapState state, Vec3d pollinationCenter,
            int stage) {
        super(state);
    ...
    private void onTick20() {
        ...
        if (this.stage == 0) {
            this.stateManager.recordAgentEvent("activity_narration", Map.of(
                    "text", "See those buds? The beehive ranks them by similarity to your flower!"));
        }
    ...
    private void onTick210() {
        ...
        if (this.stage == 0) {
            this.stateManager.recordAgentEvent("activity_narration", Map.of(
                    "text", "Flowers are about to bloom! Watch what happens to garden diversity!"));
        }
    }
    ...
    private void onPollinationEnd() {
        ...
        if (this.activityShouldEnd()) {
            ...
            this.stateManager.recordAgentEvent("activity_narration", Map.of(
                    "text", "You just experienced the filter bubble effect!"));
        } else {
            this.nextState = new MeetBip1PollinationReadyState(this, this.stage + 1);
        }
        ...
        if (this.stage == 0) {
            this.stateManager.recordAgentEvent("activity_narration", Map.of(
                    "text", "New flowers grew, some died. What happened to diversity? Check the clocks!"));
        }
    }
